...
def train_model(model):
    ...
    f = open("cuore_vgg_leaf.txt","w+")
    f.write("Cuore VGG16 Leaf \n")
    f.close()
...
def test_model(test_ds, model):
    ...
    #### TEST SIGNLE FILE ####
    '''raw = tf.io.read_file('../cuore_a0007_11.wav')
...
class CustomCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        f = open("cuore_vgg_leaf.txt","a+")
        f.write("*********** Predizione e Truth **************** Epoca: %d \n" % epoch)
        for num in y_pred:
            f.write(str(num))
            f.write(", ") 
        f.write("\n")
