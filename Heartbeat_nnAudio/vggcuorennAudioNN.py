...
def test(model, epoch, test_loader):
    ...
    f = open("cuore_vgg_nnaudio.txt", "a+")
...
def train_test(modello, ottimizzatore, perdita):
    ...
    f = open("cuore_vgg_nnaudio.txt", "w+")
    f.write("Heartbeat VGG16 nnAudio \n")
    f.close()
    ...
    torch.save(model, "/home/alessandro/venv/Modelnn/modello_cuore.pth")
    print("model saved successfully!")



    #spettrogramma post-training
    '''test, srate = torchaudio.load('../cuore_a0007_11.wav', num_frames = 4000)

    post = model.mel(test.cuda())
    post = torch.squeeze(post)
    plt.imshow(post.cpu().detach().numpy(), origin='lower')
