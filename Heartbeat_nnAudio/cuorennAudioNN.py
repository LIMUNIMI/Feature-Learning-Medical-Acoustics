...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.mel = features.MelSpectrogram(n_mels=128, hop_length=10, win_length= 30, n_fft=512, 
                              ...
                              fmin=25, fmax=1000, sr=2000, trainable_mel=True, trainable_STFT=True, verbose=True,
                              htk=True)
        
        self.effB0 = EfficientNet.from_name('efficientnet-b0', include_top=False, drop_connect_rate=0.5, num_classes=2)
        
        self.flat = torch.nn.Flatten()
        self.lin = torch.nn.Linear(1280, 2)
...
def train(model, epoch, log_interval, train_loader, val_loader, optimizer, loss_function):
    ...
    for data, labels in val_loader:
        ...
        target_val = model(data)
        loss = loss_function(target_val,labels)
...
def test(model, epoch, test_loader):
    ...
    for data, target in test_loader:
        ...
        pred = get_likely_index(torch.nn.Sigmoid()(output))
    ...
    accuracy = balanced_accuracy_score(targetlist.numpy(), predlist.numpy())
    print('Balanced Accuracy: %f' % accuracy)
    
    f = open("cuore_eff_nnaudio.txt", "a+")
    f.write("***************** Predizione ******** Epoca: %d \n" % epoch)

    for num in predlist.numpy(): 
        f.write(str(num))
        f.write(", ")
    f.write("\n")
...
def train_test(modello, ottimizzatore, perdita):
    ...
    f = open("cuore_eff_nnaudio.txt", "w+")
    f.write("Heartbeat EfficientNet nnAudio \n")
    f.close()
    ...
    #plt.ylabel("loss")
    #plt.legend()
    #plt.savefig("cuorennLoss.png")

    torch.save(model, "/Modelnn/modello_cuore.pth")
    print("model saved successfully!")



    #spettrogramma post-training
    '''testaudio, srate = torchaudio.load('../cuore_a0016_9.wav', num_frames = 4000)
    ...
    plt.savefig("trained_nnAudio_cuore.png")'''
