...
class MIOClassifier(tf.keras.Model):
  ...
  def call(self, inputs: tf.Tensor):
        output = inputs
        output = self._frontend(output, training=False) #(batch_size, time_frames, freq_bins) 
...
def train(num_epochs: int = EPOCHS,
    ...
    stop_callbacks=tf.keras.callbacks.EarlyStopping(verbose=1, patience=5)
...
def train_model(model):
    ...
    f = open("resp_eff_leaf.txt","w+")
    f.write("Respiratory Efficientnet Leaf \n")
    f.close()
