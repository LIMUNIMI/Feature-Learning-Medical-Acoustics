...
def createDS():
    ...
    for x in range(len(file_path)):
        ...
        tf_audio, _ = tf.audio.decode_wav(raw_audio, desired_samples=8000)
...
class MIOClassifier(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self._vgg = tf.keras.applications.VGG16(include_top=True, weights=None, input_shape=(224,224,1), classes=2)
    ...
    for layer in self._vgg.layers:
        ...
        if layer.name in ['fc1', 'fc2']:
            updated_model.add(tf.keras.layers.Dropout(.5))

    self._vgg = updated_model


  def call(self, inputs: tf.Tensor):
        output = inputs
        output = self._frontend(output, training=True) #(batch_size, time_frames, freq_bins) 
        ...
        return self._vgg(output)



def build_model():
    sample_rate = 4000
    leaf_complex_conv_init = initializers.GaborInit(sample_rate=sample_rate, min_freq=100., max_freq=2000.)
...
def train_model(model):
    ...
    f = open("resp_vgg_leaf.txt","w+")
    f.write("Respiratory VGG16 Leaf \n")
    f.close()
...
def test_model(test_ds, model):
    ...
    plt.ylabel('Label')
    plt.savefig("vggleafCM.png")'''
    


    ##### post trained spectrogram
    '''raw = tf.io.read_file('../resp_121_1b1_Tc_sc_Meditron_1.wav')
    tf_au, _ = tf.audio.decode_wav(raw, desired_samples=8000)
    ...
    plt.savefig("trained_Leaf_resp.png")'''
